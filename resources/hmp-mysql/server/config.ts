...
import type { MySqlConfig } from "./internal";
...
function parseConnectionUrl(value: unknown): { host?: string; port?: string; user?: string; password?: string; database?: string } {
    if (!value) return {};
...
function readJsonConfig(configPath: string): { exists: boolean; value: Record<string, unknown> } {
    if (!fs.existsSync(configPath)) return { exists: false, value: {} };
    // PowerShell 5.1 and some Windows editors prefix UTF-8 JSON with a BOM.
    // JSON.parse does not accept it even though the file is otherwise valid JSON.
...
function loadConfig(options: { env?: NodeJS.ProcessEnv; cwd?: string; configPath?: string } = {}): MySqlConfig {
...
export = { DEFAULT_CONFIG_PATH, loadConfig, parseConnectionUrl };
// TypeScript source.
