  ...
  const service = {
    api: {
      async add(
        ...
        if (fail.has(name))
          throw Object.assign(new Error(`Unknown item '${name}'`), {
            code: "HMP_INVENTORY_ITEM_UNKNOWN",
          });
        added.push({
          player: typeof target === "object" ? target.id : Number(target),
          name,
          amount,
          options,
        });
...
test("startingItems must be an array", () => {
  assert.throws(
    () => normalizeStartingItems({ name: "x" }),
    /startingItems must be an array/,
  );
});
...
test("grants only a newly-created character, once, after its inventory is loaded", async () => {
  ...
  const grant = createStartingItemsGrant(
    normalizeStartingItems([
      "housebroom",
      { name: "sealed_letter", amount: 2, metadata: { sealed: true } },
    ]),
    {
      inventory: service,
  ...
  assert.deepStrictEqual(added, [
    { player: 7, name: "housebroom", amount: 1, options: { identified: true } },
    {
      player: 7,
      name: "sealed_letter",
      amount: 2,
      options: { identified: true, metadata: { sealed: true } },
    },
  ]);
});
  ...
  const grant = createStartingItemsGrant(
    normalizeStartingItems(["sealed_letter"]),
    {
      ...
      ready: () =>
        ready
          ? Promise.resolve()
          : Promise.reject(new Error("hmp-mysql is not ready")),
      logger: silentLogger,
    },
  );
  const created = payload(12);
